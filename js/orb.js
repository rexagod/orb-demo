    var primary_image = new Image();
    primary_image.src = "small.jpg";
    var sec_image = new Image();
    sec_image.src = "big.jpg";
    ...
    window.onload = function() {
        ...
        cols = primary_image.width;
        rows = primary_image.height;
        c.width = cols;
        c.height = rows;
        ctxx.drawImage(primary_image, 0, 0, primary_image.width, primary_image.height);
        $('#myCanvas').hide();
    ...
    $(window).load(function() {
        ...
        var demo_opt = function() {
            ...
            this.train_pattern = function() {
                ...
                //maintain ratio
                var sc0 = Math.min(max_pattern_size / primary_image.width, max_pattern_size / primary_image.height);
                new_width = (primary_image.width * sc0) | 0;
                new_height = (primary_image.height * sc0) | 0;

                var c = document.getElementById("myCanvas");
                var ctxx = c.getContext("2d");
                var imgData = ctxx.getImageData(0, 0, primary_image.width, primary_image.height);
                var imgg = new jsfeat.matrix_t(primary_image.width, primary_image.height, jsfeat.U8_t | jsfeat.C1_t);
                jsfeat.imgproc.grayscale(imgData.data, primary_image.width, primary_image.height, imgg);
        ...
        function tick() {
            ...
            var data_u32 = new Uint32Array(primary_imageData.data.buffer);
            ...
            ctx.putImageData(primary_imageData, 0, 0);
        ...
        // central difference using primary_image moments to find dominant orientation
        ...
        function render_mono_primary_image(src, dst, sw, sh, dw) {
            var alpha = (0xff << 24);
            for (var i = 0; i < sh; ++i) {
                for (var j = 0; j < sw; ++j) {
                    var pix = src[i * sw + j];
